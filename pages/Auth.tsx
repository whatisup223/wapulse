...
import { MessageSquare, Mail, Lock, Eye, EyeOff, ArrowRight, ArrowLeft, Home, Globe, Moon, Sun } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin, language, onLanguageChange, isDarkMode, onThemeToggle, onBackToHome }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Back to Home */}
            <button
              onClick={onBackToHome}
              className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors font-semibold"
            >
              ...
              {isRegister && !isForgotPassword && (
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#128C7E] transition-all"
