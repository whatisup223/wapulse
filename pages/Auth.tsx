        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={(e) => { e.preventDefault(); onLogin(); }} className="space-y-6">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center px-1">
                ...
                {!isRegister && (
                  <button type="button" className="text-xs font-bold text-[#128C7E] hover:underline">
            ...
            >
              {isRegister ? (isRtl ? 'إنشاء حساب' : 'Create Account') : (isRtl ? 'تسجيل الدخول' : 'Sign In')}
          ...
          <div className="pt-4 text-center">
            <p className="text-sm text-slate-500">
              {isRegister ? (isRtl ? 'لديك حساب بالفعل؟' : 'Already have an account?') : (isRtl ? 'ليس لديك حساب؟' : "Don't have an account?")}{' '}
              <button 
                onClick={() => setIsRegister(!isRegister)}
                className="font-bold text-[#128C7E] hover:underline"
              >
                {isRegister ? (isRtl ? 'تسجيل الدخول' : 'Login') : (isRtl ? 'سجل الآن' : 'Register now')}
              </button>
            </p>
          </div>
        </div>
